<template>
  <div
    class="max-w-2xl md:max-w-xl lg:max-w-3xl p-5 md:px-12 lg:px-8 lg:py-8 mx-auto shadow-md text-darkSky dark:text-white bg-white dark:shadow-none dark:bg-semiDark rounded-xl"
  >
    <div
      class="grid user-layout-mobile md:user-layout-tablet lg:user-layout-desktop"
    >
      <img
        class="w-32 w-32 rounded-full lg:row-span-2"
        :src="user.avatar_url"
        :alt="user.login"
      />
      <!-- general info -->
      <div
        class="general col-start-2 col-span-3 pl-6 lg:pl-0 space-y-1 md:space-y-0 lg:col-start-2 lg:col-end-4"
      >
        <h2 class="text-xl md:text-2xl font-bold">{{ user.login }}</h2>
        <p class="text-base text-lightBlue">@{{ user.login }}</p>
      </div>
      <div
        class="joined-date text-sm col-start-2 col-span-3 row-start-2 pl-6 lg:pl-0 lg:col-start-4 lg:row-start-1 self-center"
      >
        <p class="whitespace-wrap lg:text-end">
          Joined {{ convertDateFormat(user.created_at) }}
        </p>
      </div>
      <div class="bio col-span-3 pt-6 lg:col-start-2 col-span-5">
        <p class="h-min">
          bio Lorem ipsum dolor sit amet, consectetur adipisicing elit. Optio
          veritatis facilis cumque explicabo at suscipit quia illo ab error
          quibusdam.
        </p>
      </div>
      <div
        class="stats flex items-center justify-around col-start-1 col-span-3 row-start-4 row-end-5 lg:row-start-3 lg:row-end-4 lg:col-start-2 lg:col-span-5 dark:bg-dark bg-lightBg rounded-lg mt-8 mb-4 text-sm"
      >
        <div class="space-y-2">
          <span>Repos</span>
          <h3
            class="text-center font-extrabold text-black dark:text-white text-lg md:text-xl"
          >
            {{ user.public_repos }}
          </h3>
        </div>
      ...
      <!--  -->
      <div
        class="pt-0 grid md:grid-cols-2 md:grid-rows-2 gap-4 flex-wrap col-start-1 col-span-3 row-start-5 self-center lg:row-start-4 lg:col-start-2 lg:col-span-5"
      >
        <div class="space-x-3 flex items-center gap-x-0">
          <svg
            id="location-icon"
            height="20"
            width="25"
        ...
        <div class="space-x-4 items-center gap-x-0 flex">
          ...
          <p :class="{ 'text-darkSky': !user.twitter_username }">
...
<script setup>
import { convertDateFormat } from "@/utils/formatDate.js";
