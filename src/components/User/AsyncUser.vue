<template>
  <div
    class="max-w-2xl mx-auto shadow-md text-darkSky dark:text-white bg-white dark:shadow-none dark:bg-semiDark p-5 rounded-xl"
  >
    <div class="grid user-layout">
      <img
        class="w-32 w-32 rounded-full"
        :src="user.avatar_url"
        :alt="user.login"
      />
      <!-- general info -->
      <div class="general col-start-2 col-span-3 pl-6 space-y-1">
        <h2 class="text-xl font-bold">{{ user.login }}</h2>
        <p class="text-base text-[#3566d1]">@{{ user.login }}</p>
      </div>
      <div class="joined-date text-sm col-start-2 col-span-3 row-start-2 pl-6">
        <p>Joined {{ convertDateFormat(user.created_at) }}</p>
      </div>
      <div class="bio col-span-3 pt-6">
        <p class="h-min">
      ...
      <div
        class="stats flex items-center justify-center col-start-1 col-span-3 row-start-4 row-end-5 dark:bg-dark bg-lightBg rounded-lg mt-8 mb-4 text-sm"
      >
        <div class="flex gap-8">
          <div class="space-y-2">
            <span>Repos</span>
            <h3
              class="text-center font-extrabold text-black dark:text-white text-lg"
            >
              {{ user.public_repos }}
            </h3>
          </div>
      ...
      <!--  -->
      <div
        class="location pt-0 grid md:grid-cols-2 md:justify-items-center md:grid-rows-2 gap-4 flex-wrap col-start-1 col-span-3 row-start-5 self-center"
      >
